...
users_time_result = zeros(3, max_user_num);
...
for i = 1 : max_user_num
    ...
    users_time_result(1, i) = sum(sum(visited_time_least_time)) / i;
    users_time_result(2, i) = sum(sum(visited_time_no_balance)) / i;
    users_time_result(3, i) = sum(sum(visited_time_with_balance)) / i;
